...
class bb_handler {
 public:
  ...
  auto bb_ref() -> peanuts::bb& { return *bb_; }
...
#else
#warning \
    "PEANUTS_USE_AGG_READ is not defined. This may cause performance degradation."
              .pread(
#endif
 ...
 private:
  std::reference_wrapper<peanuts::rpm> rpm_ref_;
...
#ifndef PEANUTS_USE_DEFERRED_OPEN
#warning \
    "PEANUTS_USE_DEFERRED_OPEN is not defined. This may cause performance degradation."
    auto meta = ino_and_size{0, -1};
    auto file = peanuts::deferred_file{pathname, flags, mode};
    try {
      file.open();
    ...
    if (myrank == 0) {
      fmt::print("peanuts::elapsed::open: {}\n", elapsed_open.count()); 
      fmt::print("peanuts::elapsed::sync_extent: {}\n", elapsed_sync.count());
    }
#endif
    return handler;
  }
