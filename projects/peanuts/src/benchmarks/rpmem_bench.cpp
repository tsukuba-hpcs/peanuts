...
auto main(int argc, char* argv[]) -> int try {
  ...
  peanuts::topology topo{};
  peanuts::rpm rpm{topo, parsed["path"].as<std::string>(),
                 utils::round_up_pow2(block_size, rpm::pmem_alignment) *
                     topo.intra_size()};
  // std::cout << utils::make_inspector(rpm) << std::endl;
  peanuts::rpm_local_block rpm_block{rpm};
  ...
  auto rpm_blocks = peanuts::rpm_blocks{rpm};
  auto remote_block = peanuts::rpm_remote_block{rpm_blocks, target_rank};
