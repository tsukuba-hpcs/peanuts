...
struct adl_serializer<bench_stats> {
  static void to_json(ordered_json& j, const bench_stats& stats) {
    ...
    j["ops_per_sec"] =
        stats.wf.n() / peanuts::utils::tsc::to_msec(stats.elapsed_cycles) * 1000;
    j["n"] = stats.wf.n();
...
class time_window {
 private:
  ...
  peanuts::utils::welford& wf_;

 public:
  time_window(uint64_t window_cycles, peanuts::utils::welford& wf)
      : t_window_start_(peanuts::utils::tsc::get()),
        t_prev_(t_window_start_),
        t_now_(t_window_start_),
        window_cycles_(window_cycles),
        wf_(wf) {}

  void update_time() { t_now_ = peanuts::utils::tsc::get(); }
...
std::vector<bench_stats> aggregate_status_in_same_window(
  ...
  while (remain) {
    ...
    remain = false;
    std::vector<peanuts::utils::welford> welfords;
    ...
    if (remain) {
      result.push_back(
          {peanuts::utils::welford::from_range(welfords.begin(), welfords.end()),
           max_cycles});
    }
    ++window_id;
  }
...
auto main(int argc, char const* argv[]) -> int {
  ...
  peanuts::utils::tsc::calibrate();

  const auto window_cycles = peanuts::utils::tsc::cycles_per_msec() * window;

  ordered_json bench_result = {
      {"version", PEANUTS_VERSION},
      {"timestamp", fmt::format("{:%FT%TZ}", std::chrono::system_clock::now())},
      {"cycles_per_msec", peanuts::utils::tsc::cycles_per_msec()},
      {"nthreads", nthreads},
      {"iter", iter},
  ...
  peanuts::utils::sense_barrier barrier(nthreads);

  for (size_t tid = 0; tid < nthreads; ++tid) {
    threads.emplace_back([&, tid]() {
      using peanuts::utils::tsc;
      peanuts::utils::cpu_affinity_manager cpu_affinity(tid);

      peanuts::extent_tree tree;

      if (add) {
        peanuts::extent ex(0, extent_size);
        peanuts::utils::welford wf;
        barrier.wait();
        ...
        if (tid == 0) {
          ...
          // std::cout << peanuts::utils::make_inspector(welfords) << std::endl;
