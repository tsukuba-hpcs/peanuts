...
#include "peanuts.hpp"
#include "peanuts/inspector/mpi.hpp"
...
TEST_CASE("mpi::mpi_error_category") {
  peanuts::mpi::mpi_error_category cat;
...
TEST_CASE("mpi::runtime") {
  peanuts::mpi::runtime runtime;
  peanuts::mpi::runtime r2 = std::move(runtime);
  peanuts::mpi::runtime r3{std::move(r2)};
  CHECK(peanuts::mpi::runtime::is_initialized() == true);
  CHECK(peanuts::mpi::runtime::is_finalize() == false);
  CHECK(peanuts::mpi::runtime::query_thread_support() == MPI_THREAD_MULTIPLE);
  CHECK(peanuts::mpi::runtime::is_thread_main() == true);
  MESSAGE(peanuts::mpi::runtime::processor_name());
  CHECK(peanuts::mpi::runtime::processor_name().size() > 0);
}

TEST_CASE("group") {
  const auto& comm = peanuts::mpi::comm::world();
...
TEST_CASE("win") {
  ...
  MESSAGE(peanuts::utils::to_string(info));
...
TEST_CASE("info class: Initialize from initializer list") {
  peanuts::mpi::info my_info = {{"key1", "value1"}, {"key2", "value2"}};
...
TEST_CASE("aint") {
  ...
  SUBCASE("Construct from void pointer") {
    int data;
    peanuts::mpi::aint a(&data);
    CHECK(a.native() == peanuts::mpi::aint::to_aint(&data));
  }
  ...
  SUBCASE("Operator +") {
    peanuts::mpi::aint a(40);
    peanuts::mpi::aint b(2);
    peanuts::mpi::aint result = a + b;
