...
class ring_tracker {
 public:
  ...
  size_t first_segment_size(uint64_t lsn, size_t size) const {
  ...
  bool is_wrapping(uint64_t lsn, size_t size) const {
  ...
  uint64_t used_capacity() const { return head_lsn_ - tail_lsn_; }

  uint64_t free_capacity() const { return ring_size_ - used_capacity(); }
